...
<br><br><br><br>
  ...
  <a onClick="return confirm('Anda Yakin Ingin Keluar?')" href="<?php echo $logoutAction ?>" style="text-decoration:none"><li class="w3-hover-pale-blue"><i class="fa fa-sign-out fa-fw"></i> Keluar</li></a>
        ...
        <div class="w3-col m8 l9 w3-padding-large"><!-- InstanceBeginEditable name="EditRegion1" --><div class="w3-large w3-border-bottom" style="padding-bottom:8px;">Beranda</div>
        ...
        <div style="margin-top:12px;">
        	<div class="w3-row">
                ...
                <?php
		error_reporting(0);
	   $dataParkir = mysql_num_rows(mysql_query("SELECT * FROM tb_parkir"));
	   $dataJenis = mysql_num_rows(mysql_query("SELECT * FROM tb_jenis_kendaraan"));
	   $dataBiaya = mysql_num_rows(mysql_query("SELECT * FROM tb_biaya_parkir"));
	   $dataPetugas = mysql_num_rows(mysql_query("SELECT * FROM tb_petugas"));
	   
	   $sm = mysql_num_rows(mysql_query("SELECT * FROM tb_parkir WHERE date(masuk)='$tglSekarang' AND jenis_kendaraan='Sepeda Motor'"));
	   $mp = mysql_num_rows(mysql_query("SELECT * FROM tb_parkir WHERE date(masuk)='$tglSekarang' AND jenis_kendaraan='Mobil Penumpang'"));
	   $mb = mysql_num_rows(mysql_query("SELECT * FROM tb_parkir WHERE date(masuk)='$tglSekarang' AND jenis_kendaraan='Mobil Barang'"));
	   $dl = mysql_num_rows(mysql_query("SELECT * FROM tb_parkir WHERE date(masuk)='$tglSekarang' AND jenis_kendaraan!='Sepeda Motor' AND jenis_kendaraan!='Mobil Penumpang' AND jenis_kendaraan!='Mobil Barang'"));
	   ?>
       <ul class="w3-ul w3-border w3-small">
                	<li class="w3-pale-green w3-center">
                    Selamat Datang Di Aplikasi<br>
<font style="font-size:24px"><b>BIG PARKING</b></font><br>
PT. Batavia Indo Global
                    </li>
                  </ul>
                <ul style="margin-top:10px;" class="w3-ul w3-border w3-small">
                	<li class="w3-pale-yellow"><strong>Info Data</strong></li>
                	<li>Data Keseluruhan Parkir<span class="w3-right w3-tag"><?php echo $dataParkir ?></span></li>
                    <li>Data Jenis Kendaraan<span class="w3-right w3-tag"><?php echo $dataJenis ?></span></li>
                    <li>Data Biaya Parkir<span class="w3-right w3-tag"><?php echo $dataBiaya ?></span></li>
                    <li>Data Petugas<span class="w3-right w3-tag"><?php echo $dataPetugas ?></span></li>
                    
                    
                    
                    
                </ul>
                <ul class="w3-ul w3-border w3-small" style="margin-top:10px;">
                	<li class="w3-pale-red w3-center">
                    Silahkan Buka <b>Menu Bantuan</b><br>
Untuk Penggunaan Aplikasi Ini
                    </li>
                  </ul>
                </div>
                <div class="w3-col s12 m5 l7" style="padding-left:15px;"><div class="w3-border w3-padding">
                <center><strong>STATISTIK PARKIR</strong><br>
                <?php
		$year = substr($tglSekarang,0,4);
		$mont = substr($tglSekarang,5,2);
		$day = substr($tglSekarang,8,2);
		 ?> 
TANGGAL <?php echo $day."-".$mont."-".$year ?>
</center>
        <div class="container">
        ...
        <script>
            ...
            var myChart = new Chart(ctx, {
                ...
                data: {
                    ...
                    labels: ["Sepeda Motor","Mobil Penumpang","Mobil Barang","Lainnya"],
                    datasets: [{
                            label: 'Jumlah Yang Parkir ',
                            data: ["<?php echo $sm ?>","<?php echo $mp ?>","<?php echo $mb ?>","<?php echo $dl ?>"],
                            backgroundColor: [
