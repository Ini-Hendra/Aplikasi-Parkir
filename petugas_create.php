...
if (isset($_POST[$MM_flag])) {
  $MM_dupKeyRedirect="petugas_error.php";
  $loginUsername = $_POST['username'];
  $LoginRS__query = sprintf("SELECT username FROM tb_petugas WHERE username=%s", GetSQLValueString($loginUsername, "text"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO tb_petugas (id_petugas, nama_lengkap, username, password, tgl_daftar) VALUES (%s, %s, %s, %s, %s)",
...
$colname_rsPetugasCreate = "-1";
if (isset($_GET['id_petugas'])) {
  $colname_rsPetugasCreate = $_GET['id_petugas'];
}
mysql_select_db($database_koneksi, $koneksi);
$query_rsPetugasCreate = sprintf("SELECT * FROM tb_petugas WHERE id_petugas = %s", GetSQLValueString($colname_rsPetugasCreate, "text"));
$rsPetugasCreate = mysql_query($query_rsPetugasCreate, $koneksi) or die(mysql_error());
$row_rsPetugasCreate = mysql_fetch_assoc($rsPetugasCreate);
$totalRows_rsPetugasCreate = mysql_num_rows($rsPetugasCreate);
$IDPEtugas = "123456789123456789123456789123456789";
?>
...
<br><br><br><br>
  ...
  <a onClick="return confirm('Anda Yakin Ingin Keluar?')" href="<?php echo $logoutAction ?>" style="text-decoration:none"><li class="w3-hover-pale-blue"><i class="fa fa-sign-out fa-fw"></i> Keluar</li></a>
        ...
        <div class="w3-col m8 l9 w3-padding-large"><!-- InstanceBeginEditable name="EditRegion1" --><script src="SpryAssets/SpryValidationPassword.js" type="text/javascript"></script>
<link href="SpryAssets/SpryValidationPassword.css" rel="stylesheet" type="text/css" />
<link href="SpryAssets/SpryValidationConfirm.css" rel="stylesheet" type="text/css" />
<script src="SpryAssets/SpryValidationConfirm.js" type="text/javascript"></script>

<div class="w3-large w3-border-bottom" style="padding-bottom:8px;"><a onClick="document.getElementById('loading').style.display='block'" href="petugas_read.php"><i class="fa fa-arrow-left"></i></a> &nbsp;&nbsp;Tambah Petugas Parkir</div>
...
<div style="margin-top:8px">
  ...
  <table width="100%" align="center">
    <tr valign="baseline" style="display:none">
      <td align="left" valign="middle" nowrap="nowrap">Id_petugas:</td>
      <td><input type="text" name="id_petugas" value="<?php echo substr(str_shuffle($IDPEtugas),0,10) ?>" size="32" /></td>
    </tr>
    <tr valign="baseline">
      <td align="left" valign="middle" nowrap="nowrap">Nama Petugas</td>
      <td><input type="text" class="w3-input w3-border w3-small" required placeholder="" name="nama_lengkap" value="" size="32" /></td>
    </tr>
    <tr valign="baseline">
      <td align="left" valign="middle" nowrap="nowrap">Username</td>
      <td><input class="w3-input w3-border w3-small" required type="text" name="username" value="" size="32" /></td>
    </tr>
    <tr valign="baseline">
      <td align="left" valign="middle" nowrap="nowrap">Password</td>
      <td><span id="sprypassword1">
      <input class="w3-input w3-border w3-small" style="background-color:white" required type="password" name="password" id="password" />
      <span class="passwordRequiredMsg">A value is required.</span>
      <span class="passwordMinCharsMsg" style="border:none;font-size:10px;">Password Minimal 6 Karakter</span>
      </span></td>
    </tr>
    <tr valign="baseline">
      <td align="left" valign="middle" nowrap="nowrap">Ulangi Password</td>
      <td><span id="spryconfirm1">
        <input class="w3-input w3-border w3-small" style="background-color:white" required type="password" name="password1" id="password1" />
      <span class="confirmRequiredMsg">A value is required.</span>
      <span class="confirmInvalidMsg" style="border:none;font-size:10px;">Ulangi Password Dengan Benar</span></span></td>
    </tr>
    <tr valign="baseline" style="display:none">
      <td align="left" valign="middle" nowrap="nowrap">Tgl_daftar:</td>
      <td><input type="text" name="tgl_daftar" value="<?php echo date('Y-m-d H:i:s') ?>" size="32" /></td>
    </tr>
...
<p>&nbsp;</p>
<?php
mysql_free_result($rsPetugasCreate);
?>
