...
$query_rsPetugasRead = "SELECT * FROM tb_petugas ORDER BY tgl_daftar DESC";
$rsPetugasRead = mysql_query($query_rsPetugasRead, $koneksi) or die(mysql_error());
$row_rsPetugasRead = mysql_fetch_assoc($rsPetugasRead);
$totalRows_rsPetugasRead = mysql_num_rows($rsPetugasRead);
?>
...
<div class="w3-large w3-border-bottom" style="padding-bottom:8px;">Data Petugas Parkir</div>
<?php if ($totalRows_rsPetugasRead > 0) { // Show if recordset not empty ?>
  <div class="w3-row">
 	<div class="w3-col l3" style="padding-right:4px;"><a style="margin-top:8px; height:36px;" onClick="document.getElementById('loading').style.display='block'" class="w3-btn w3-block w3-green w3-small" href="petugas_create.php"><i class="fa fa-plus fa-fw"></i> Tambah Data</a></div>
	<div class="w3-col l3" style="padding-right:4px; padding-left:4px;"><input oninput="w3.filterHTML('#petugas', '.petugas', this.value)" placeholder="Pencarian Data" class="w3-input w3-border w3-center w3-small" style="outline:none;margin-top:8px;" autofocus></div>
    <div class="w3-col l6" style="padding-left:4px;"><div class="w3-border w3-border-yellow w3-pale-yellow w3-padding w3-small" style="margin-top:8px;">
        Total Data<span class="w3-right"><?php echo $totalRows_rsPetugasRead ?></span>
        </div></div>
</div>

       <div class="w3-responsive"> 
  <table class="w3-table w3-striped w3-small w3-hoverable w3-bordered w3-border" id="petugas" style="margin-top:8px;">
      <tr style="font-weight:bold" class="w3-hover-none">
      <td class="w3-border-right w3-center">No</td>
      <td class="w3-border-right">Nama Petugas</td>
      <td class="w3-border-right w3-center">Username</td>
      <td class="w3-border-right w3-center">Password</td>
      <td class="w3-border-right w3-center">Tanggal Daftar</td>
      <td class="w3-center">Opsi</td>
    </tr>
    <?php $nomor = 1; do { ?>
      <tr class="petugas">
        <td class="w3-border-right w3-center"><?php echo $nomor++; ?></td>
        <td class="w3-border-right"><?php echo $row_rsPetugasRead['nama_lengkap']; ?></td>
        <td class="w3-border-right w3-center"><?php echo $row_rsPetugasRead['username']; ?></td>
        <td class="w3-border-right w3-center"><?php echo $row_rsPetugasRead['password']; ?></td>
        <td class="w3-border-right w3-center"><?php
		$year = substr($row_rsPetugasRead['tgl_daftar'],0,4);
		$mont = substr($row_rsPetugasRead['tgl_daftar'],5,2);
		$day = substr($row_rsPetugasRead['tgl_daftar'],8,2);
		$jam = substr($row_rsPetugasRead['tgl_daftar'],11,5);
		 ?> <?php echo $day."-".$mont."-".$year." ".$jam; ?>
		 </td>
        <td class="w3-center"><a class="w3-tag w3-small w3-red" style="text-decoration:none" onclick="return confirm('Anda Yakin Ingin Menghapus?\nPetugas : <?php echo $row_rsPetugasRead['nama_lengkap']; ?>')" href="petugas_delete.php?id_petugas=<?php echo $row_rsPetugasRead['id_petugas']; ?>">Hapus</a></td>
      </tr>
      <?php } while ($row_rsPetugasRead = mysql_fetch_assoc($rsPetugasRead)); ?>
  </table>
  </div>
  <br>
...
<?php
mysql_free_result($rsPetugasRead);
?>
<?php if ($totalRows_rsPetugasRead == 0) { // Show if recordset empty ?>
  <div class="w3-row">
 	<div class="w3-col l3" style="padding-right:4px;"><a style="margin-top:8px; height:36px;" class="w3-btn w3-block w3-green w3-small" href="petugas_create.php"><i class="fa fa-plus fa-fw"></i> Tambah Data</a></div>
	<div class="w3-col l3" style="padding-right:4px; padding-left:4px;"><input oninput="w3.filterHTML('#petugas', '.petugas', this.value)" placeholder="Pencarian Data" class="w3-input w3-border w3-center w3-small" style="outline:none;margin-top:8px;" autofocus></div>
    <div class="w3-col l6" style="padding-left:4px;"><div class="w3-border w3-border-yellow w3-pale-yellow w3-padding w3-small" style="margin-top:8px;">
        Total Data<span class="w3-right"><?php echo $totalRows_rsPetugasRead ?></span>
        </div></div>
