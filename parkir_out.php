...
<div class="w3-large w3-border-bottom" style="padding-bottom:8px;">Input Keluar Parkir</div>
        
        
        
        
<form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="formMasuk" style="display:none">
  <table align="center">
    <tr valign="baseline">
    ...
    <tr valign="baseline">
      ...
      <td><input type="text" name="no_polisi" value="" size="32" /></td>
      </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Jenis_kendaraan:</td>
      <td><fieldset style="margin-top:8px;">
  ...
  <button type="button" onClick="document.getElementById('formMasuk').submit();document.getElementById('cetakMasuk').click()" class="w3-btn w3-small w3-block w3-green"><i class="fa fa-check-circle fa-fw"></i> Masuk</button>
  <input type="hidden" name="MM_insert" value="form1" />
</form>
<a id="cetakMasuk" style="display:none" href="cek.php" 
  target="popup" 
...
<?php if ($totalRows_rsParkirRead > 0) { // Show if recordset not empty ?>
  <table border="1" style="display:none">
    ...
    <?php do { ?>
      <tr>
        <td><?php echo $row_rsParkirRead['id_parkir']; ?></td>
        <td><?php echo $row_rsParkirRead['no_polisi']; ?></td>
        <td><?php echo $row_rsParkirRead['jenis_kendaraan']; ?></td>
        <td><?php echo $row_rsParkirRead['masuk']; ?></td>
        <td><?php echo $row_rsParkirRead['keluar']; ?></td>
        <td><?php echo $row_rsParkirRead['durasi']; ?></td>
        <td><?php echo $row_rsParkirRead['biaya']; ?></td>
        <td><?php echo $row_rsParkirRead['status']; ?></td>
        </tr>
...
<?php if ($totalRows_rsParkirRead == 0) { // Show if recordset empty ?>
  ...
  Silahkan Masukan Kode Tiket<br>
Yang Tertera Pada Tiket
</div>
<form method="post" action="parkir_out.php">
<div style="margin-top:10px;">
<input autofocus class="w3-input w3-center w3-border w3-small" placeholder="Kode Tiket" type="text" name="kodeNya" value="" required />
</div>
	<div>
    <button style="margin-top:10px;" type="submit" onClick="document.getElementById('loading').style.display='block'" class="w3-btn w3-small w3-block w3-green"><i class="fa fa-sign-out fa-fw"></i> Keluar</button>
    </div>
</form></div>
            <div class="w3-col l7 m7 s12" style="padding-left:15px;"><div style="margin-top:8px;border:1px red dashed" class="w3-small w3-padding-large">
  <?php if ($totalRows_rsParkirCari > 0) { // Show if recordset not empty ?>
    
    <?php do { ?>
    <?php
		$jk = $row_rsParkirCari['jenis_kendaraan'];
	...
	?> 
    <ul class="w3-ul w3-small w3-border" style="margin-top:10px;">
    	<li>Kode Tiket<span class="w3-right"><?php echo $row_rsParkirCari['id_parkir']; ?></span></li>
        <li>No. Polisi<span class="w3-right"><?php echo $row_rsParkirCari['no_polisi']; ?></span></li>
        <li>Jenis Kendaraan<span class="w3-right"><?php echo $jk ?></span></li>
        <li>Masuk<span class="w3-right"><?php
		$year3 = substr($row_rsParkirCari['masuk'],0,4);
		$mont3 = substr($row_rsParkirCari['masuk'],5,2);
		$day3 = substr($row_rsParkirCari['masuk'],8,2);
		$jam3 = substr($row_rsParkirCari['masuk'],11,5);
		 ?> <?php echo $day3."-".$mont3."-".$year3." ".$jam3; ?></span></li>
        <li>Keluar<span class="w3-right"><?php
		$year4 = substr(date('Y-m-d H:i:s'),0,4);
		$mont4 = substr(date('Y-m-d H:i:s'),5,2);
		$day4 = substr(date('Y-m-d H:i:s'),8,2);
		$jam4 = substr(date('Y-m-d H:i:s'),11,5);
		 ?> <?php echo $day4."-".$mont4."-".$year4." ".$jam4; ?>
    ...
    <ul class="w3-ul w3-small w3-border" style="margin-top:10px;">
    	<li>Biaya Perjam<span class="w3-right">Rp. <?php echo number_format($biayaPerjam,0,",","."); ?></span></li>
        <li>Biaya Maksimal Perhari<span class="w3-right">Rp. <?php echo number_format($biayaMaksimal,0,",","."); ?></span></li>
    </ul>
    <ul class="w3-ul w3-small w3-border" style="margin-top:10px;">
     	<li class="w3-center"><div>Total Bayar</div>
        <div style="font-size:16px;font-weight:bold"><?php if($hariNya >= "1"){
				if($jamNya >= "1" && $menitNya >= "10"){
					$sisa = ($jamNya*$biayaPerjam)+$biayaPerjam;
				} elseif($jamNya == "0" && $menitNya >= "10"){
					$sisa = $biayaPerjam;
				...
				if($jamNya >= "1" && $menitNya >= "10"){
					$totalJam = ($jamNya*$biayaPerjam)+$biayaPerjam;
					echo "Rp. ".number_format($totalJam,0,",",".");
				} elseif($jamNya == "0" && $menitNya2 <= 59) {
					echo "Rp. ".number_format($biayaPerjam,0,",",".");
			} elseif($hariNya <= "1" && $menitNya <= "10") {
				$totalJam = ($jamNya*$biayaPerjam);
				echo "Rp. ".number_format($totalJam,0,",",".");
			}
        ...
        <div style="font-size:12px">Durasi <?php echo $durasi ?></div></li>
    ...
    <form action="<?php echo $editFormAction; ?>" method="post" name="form2" id="formKeluar">
          ...
          <button type="button" style="margin-top:10px;" onClick="document.getElementById('formKeluar').submit();document.getElementById('cetakKeluar').click()" class="w3-btn w3-small w3-block w3-green"><i class="fa fa-sign-out fa-fw"></i> Bayar</button>
          <input type="hidden" name="MM_update" value="form2" />
          <input type="hidden" name="id_parkir" value="<?php echo $row_rsParkirCari['id_parkir']; ?>" />
        </form>
          <a style="display:none" id="cetakKeluar" href="cek2.php?id_parkir=<?php echo $row_rsParkirCari['id_parkir']; ?>" 
  target="popup" 
...
<?php if ($totalRows_rsParkirCari == 0) { // Show if recordset empty ?>
  <table class="w3-table w3-border" style="margin-top:8px;">
    <tr>
      <td class="w3-center">Tiket Belum Dimasukan<br>
Atau Tidak Ditemukan</td>
      </tr>
    </table>
  <?php } // Show if recordset empty ?>
</div></div>
...
<?php if ($totalRows_rsParkirKeluar > 0) { // Show if recordset not empty ?>
  <div class="w3-row" style="margin-top:10px;">
	...
	<div class="w3-col l3" style="padding-right:4px;"><input oninput="w3.filterHTML('#penga2', '.penga2', this.value)" placeholder="Pencarian Data" class="w3-input w3-border w3-center w3-small" style="outline:none;margin-top:8px;" autofocus></div>
    <div class="w3-col l9" style="padding-left:4px;"><div class="w3-border w3-border-yellow w3-pale-yellow w3-padding w3-small" style="margin-top:8px;">
        Total Data<span class="w3-right"><?php echo $totalRows_rsParkirKeluar ?></span>
        </div></div>
</div>

        
  <table class="w3-table w3-striped w3-small w3-hoverable w3-bordered w3-border" id="penga2" style="margin-top:8px;">
      ...
      <td class="w3-border-right w3-center">Biaya</td>
      <td class="w3-center">Opsi</td>
      
      </tr>
    <?php do { ?>
      <tr class="penga2">
        <td class="w3-border-right w3-center"><?php echo $row_rsParkirKeluar['id_parkir']; ?></td>
        <td class="w3-border-right"><?php echo $row_rsParkirKeluar['no_polisi']; ?></td>
        <td class="w3-border-right"><?php echo $row_rsParkirKeluar['jenis_kendaraan']; ?></td>
        <td class="w3-border-right w3-center"><?php
		$year = substr($row_rsParkirKeluar['masuk'],0,4);
		$mont = substr($row_rsParkirKeluar['masuk'],5,2);
		$day = substr($row_rsParkirKeluar['masuk'],8,2);
		$jam = substr($row_rsParkirKeluar['masuk'],11,5);
		 ?> <?php echo $day."-".$mont."-".$year." ".$jam; ?></td>
        <td class="w3-border-right w3-center"><?php
		$year2 = substr($row_rsParkirKeluar['keluar'],0,4);
		$mont2 = substr($row_rsParkirKeluar['keluar'],5,2);
		$day2 = substr($row_rsParkirKeluar['keluar'],8,2);
		$jam2 = substr($row_rsParkirKeluar['keluar'],11,5);
		 ?> <?php echo $day2."-".$mont2."-".$year2." ".$jam2; ?></td>
        <td class="w3-border-right w3-center"><?php echo $row_rsParkirKeluar['durasi']; ?></td>
        <td class="w3-border-right">Rp. <span class="w3-right"><?php echo number_format($row_rsParkirKeluar['biaya'],0,",","."); ?></span></td>
        <td class="w3-center"><a class="w3-tag w3-small w3-red" style="text-decoration:none" onClick="return confirm('Anda Yakin Ingin Menghapus?')" href="parkir_delete2.php?id_parkir=<?php echo $row_rsParkirKeluar['id_parkir']; ?>">Hapus</a></td>
        </tr>
...
<?php if ($totalRows_rsParkirKeluar == 0) { // Show if recordset empty ?>
  <div class="w3-row" style="margin-top:10px;">
    ...
    <tr>
      <td class="w3-center">Belum Ada Yang Keluar Parkiran</td>
      </tr>
    </table>
  <?php } // Show if recordset empty ?>
