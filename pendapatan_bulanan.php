...
$query_rsPendapatanBulanan = sprintf("SELECT * FROM tb_parkir WHERE MONTH(keluar) = %s AND YEAR(keluar) = %s ORDER BY keluar DESC", GetSQLValueString($colname_rsPendapatanBulanan, "date"),GetSQLValueString($colname_rsPendapatanBulanan2, "date"));
...
<br><br><br><br>
  ...
  <a onClick="return confirm('Anda Yakin Ingin Keluar?')" href="<?php echo $logoutAction ?>" style="text-decoration:none"><li class="w3-hover-pale-blue"><i class="fa fa-sign-out fa-fw"></i> Keluar</li></a>
        ...
        <div class="w3-large w3-border-bottom" style="padding-bottom:8px;">Data Pendapatan Berdasarkan Bulan & Tahun</div>
<?php if ($totalRows_rsPendapatanBulanan > 0) { // Show if recordset not empty ?>
  ...
  if($dataSektor2 === FALSE){
	...
	while($hasil_dataSektor2 = mysql_fetch_array($dataSektor2)){
		...
		$a = 0;
	  	$sql = mysqli_query($conn,"SELECT * FROM tb_parkir WHERE MONTH(keluar) = '$colname_rsPendapatanBulanan' AND YEAR(keluar) = '$colname_rsPendapatanBulanan2' AND jenis_kendaraan='$jenisKendaraanNya'");
		...
		?>  
		<tr>
      	<td colspan="6">Total Pendapatan <?php echo $jenisKendaraanNya ?> (<?php echo mysql_num_rows(mysql_query("SELECT * FROM tb_parkir WHERE MONTH(keluar) = '$colname_rsPendapatanBulanan' AND YEAR(keluar) = '$colname_rsPendapatanBulanan2' AND jenis_kendaraan='$jenisKendaraanNya'")) ?>)</td>
   ...
   ?>
      ...
      <?php
      	...
      	$a1 = 0;
	  	$sql1 = mysqli_query($conn,"SELECT * FROM tb_parkir WHERE MONTH(keluar) = '$colname_rsPendapatanBulanan' AND YEAR(keluar) = '$colname_rsPendapatanBulanan2'");
