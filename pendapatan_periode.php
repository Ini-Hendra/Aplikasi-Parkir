...
$colname_rsPendapatanPeriode = "-1";
$colname_rsPendapatanPeriode2 = "-1";
if (isset($_POST['tglAwal']) && isset($_POST['tglAkhir'])) {
  $colname_rsPendapatanPeriode = $_POST['tglAwal'];
  $colname_rsPendapatanPeriode2 = $_POST['tglAkhir'];
}
mysql_select_db($database_koneksi, $koneksi);
$query_rsPendapatanPeriode = sprintf("SELECT * FROM tb_parkir WHERE date(keluar) BETWEEN %s AND %s ORDER BY keluar DESC", GetSQLValueString($colname_rsPendapatanPeriode, "date"),GetSQLValueString($colname_rsPendapatanPeriode2, "date"));
$rsPendapatanPeriode = mysql_query($query_rsPendapatanPeriode, $koneksi) or die(mysql_error());
$row_rsPendapatanPeriode = mysql_fetch_assoc($rsPendapatanPeriode);
$totalRows_rsPendapatanPeriode = mysql_num_rows($rsPendapatanPeriode);
?>
...
<br><br><br><br>
  ...
  <a onClick="return confirm('Anda Yakin Ingin Keluar?')" href="<?php echo $logoutAction ?>" style="text-decoration:none"><li class="w3-hover-pale-blue"><i class="fa fa-sign-out fa-fw"></i> Keluar</li></a>
        ...
        <div class="w3-large w3-border-bottom" style="padding-bottom:8px;">Data Pendapatan Berdasarkan Periode Tanggal</div>
<?php if ($totalRows_rsPendapatanPeriode > 0) { // Show if recordset not empty ?>
<?php
		$year3 = substr($colname_rsPendapatanPeriode,0,4);
		$mont3 = substr($colname_rsPendapatanPeriode,5,2);
		$day3 = substr($colname_rsPendapatanPeriode,8,2);
		
		$year4 = substr($colname_rsPendapatanPeriode,0,4);
		$mont4 = substr($colname_rsPendapatanPeriode,5,2);
		$day4 = substr($colname_rsPendapatanPeriode,8,2);
		 ?> 
  <div class="w3-row">
    ...
    <div class="w3-col l9" style="padding-left:4px;"><div class="w3-border w3-border-yellow w3-pale-yellow w3-padding w3-small" style="margin-top:8px;">
        Sub Total <span class="w3-right"><?php echo $day3."-".$mont3."-".$year3 ?> Sampai <?php echo $day4."-".$mont4."-".$year4 ?></span>
        </div></div>
</div>
  ...
  <table class="w3-table w3-striped w3-small w3-hoverable w3-bordered w3-border" id="penga" style="margin-top:8px;">
    ...
    <?php $nomor = 1; do { ?>
      <tr class="penga">
        ...
        <td class="w3-border-right"><?php echo $row_rsPendapatanPeriode['no_polisi']; ?></td>
        <td class="w3-border-right"><?php echo $row_rsPendapatanPeriode['jenis_kendaraan']; ?></td>
        <td class="w3-border-right w3-center"><?php
		$year = substr($row_rsPendapatanPeriode['masuk'],0,4);
		$mont = substr($row_rsPendapatanPeriode['masuk'],5,2);
		$day = substr($row_rsPendapatanPeriode['masuk'],8,2);
		$jam = substr($row_rsPendapatanPeriode['masuk'],11,5);
		 ?> <?php echo $day."-".$mont."-".$year." ".$jam; ?>
		 </td>
        <td class="w3-border-right w3-center"><?php
		$year2 = substr($row_rsPendapatanPeriode['keluar'],0,4);
		$mont2 = substr($row_rsPendapatanPeriode['keluar'],5,2);
		$day2 = substr($row_rsPendapatanPeriode['keluar'],8,2);
		$jam2 = substr($row_rsPendapatanPeriode['keluar'],11,5);
		 ?> <?php echo $day2."-".$mont2."-".$year2." ".$jam2; ?></td>
        <td class="w3-border-right w3-center"><?php echo $row_rsPendapatanPeriode['durasi']; ?></td>
        <td>Rp.<span class="w3-right"><?php echo number_format($row_rsPendapatanPeriode['biaya'],0,",","."); ?></span></td>
      </tr>
      <?php } while ($row_rsPendapatanPeriode = mysql_fetch_assoc($rsPendapatanPeriode)); ?>
      
      
      
      
      <?php
	  include("koneksi_sum.php");
  $dataSektor2 = mysql_query("SELECT * FROM tb_jenis_kendaraan");
  if($dataSektor2 === FALSE){
	  die(mysql_error());
	  }
	while($hasil_dataSektor2 = mysql_fetch_array($dataSektor2)){
		
		$IDJen = $hasil_dataSektor2['id_jenis_kendaraan'];
		$jenisKendaraanNya = $hasil_dataSektor2['jenis_kendaraan'];
		
		
		$a = 0;
	  	$sql = mysqli_query($conn,"SELECT * FROM tb_parkir WHERE date(keluar) BETWEEN '$colname_rsPendapatanPeriode' AND '$colname_rsPendapatanPeriode2' AND jenis_kendaraan='$jenisKendaraanNya'");
	  	while($data = mysqli_fetch_array($sql)){
		  $a++;
		  $totalnya[$a] = $data['biaya'];
		  }
		?>  
		<tr>
      	<td colspan="6">Total Pendapatan <?php echo $jenisKendaraanNya ?> (<?php echo mysql_num_rows(mysql_query("SELECT * FROM tb_parkir WHERE date(keluar) BETWEEN '$colname_rsPendapatanPeriode' AND '$colname_rsPendapatanPeriode2' AND jenis_kendaraan='$jenisKendaraanNya'")) ?>)</td>
        <td>Rp.<span class="w3-right"><?php $mhri = array_sum($totalnya); echo number_format(@$mhri,0,",",".") ?></span></td>
      </tr>
        <?php
	}
   ?>
      ...
      <?php
      	...
      	$a1 = 0;
	  	$sql1 = mysqli_query($conn,"SELECT * FROM tb_parkir WHERE date(keluar) BETWEEN '$colname_rsPendapatanPeriode' AND '$colname_rsPendapatanPeriode2'");
	  	while($data1 = mysqli_fetch_array($sql1)){
		  $a1++;
		  $totalnya1[$a1] = $data1['biaya'];
		  }
		  ?>
      <tr>
      	<td colspan="6">Total Pendapatan Seluruhnya</td>
        <td>Rp.<span class="w3-right"><?php $mhri = array_sum($totalnya1); echo number_format(@$mhri,0,",",".") ?></span></td>
      </tr>
  </table>
  <br>
...
<?php
mysql_free_result($rsPendapatanPeriode);
?>
<?php if ($totalRows_rsPendapatanPeriode == 0) { // Show if recordset empty ?>
<?php
		$year3 = substr($colname_rsPendapatanPeriode,0,4);
		$mont3 = substr($colname_rsPendapatanPeriode,5,2);
		$day3 = substr($colname_rsPendapatanPeriode,8,2);
