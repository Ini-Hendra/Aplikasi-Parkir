...
$colname_rsPendapatanJenis = "-1";
if (isset($_POST['txtJenis'])) {
  $colname_rsPendapatanJenis = $_POST['txtJenis'];
}
mysql_select_db($database_koneksi, $koneksi);
$query_rsPendapatanJenis = sprintf("SELECT * FROM tb_parkir WHERE status='Keluar' AND jenis_kendaraan = %s ORDER BY keluar DESC", GetSQLValueString($colname_rsPendapatanJenis, "text"));
$rsPendapatanJenis = mysql_query($query_rsPendapatanJenis, $koneksi) or die(mysql_error());
$row_rsPendapatanJenis = mysql_fetch_assoc($rsPendapatanJenis);
$totalRows_rsPendapatanJenis = mysql_num_rows($rsPendapatanJenis);
?>
...
<br><br><br><br>
  ...
  <a onClick="return confirm('Anda Yakin Ingin Keluar?')" href="<?php echo $logoutAction ?>" style="text-decoration:none"><li class="w3-hover-pale-blue"><i class="fa fa-sign-out fa-fw"></i> Keluar</li></a>
        ...
        <div class="w3-large w3-border-bottom" style="padding-bottom:8px;">Data Pendapatan Berdasarkan Jenis Kendaraan</div>
<?php if ($totalRows_rsPendapatanJenis > 0) { // Show if recordset not empty ?>
  <div class="w3-row">
    ...
    <div class="w3-col l9" style="padding-left:4px;"><div class="w3-border w3-border-yellow w3-pale-yellow w3-padding w3-small" style="margin-top:8px;">
        Sub Total <?php echo $colname_rsPendapatanJenis ?><span class="w3-right"><?php echo $totalRows_rsPendapatanJenis ?></span>
        </div></div>
</div>
  ...
  <table class="w3-table w3-striped w3-small w3-hoverable w3-bordered w3-border" id="penga" style="margin-top:8px;">
    ...
    <?php $nomor = 1; do { ?>
      <tr class="penga">
        ...
        <td class="w3-border-right"><?php echo $row_rsPendapatanJenis['no_polisi']; ?></td>
        <td class="w3-border-right"><?php echo $row_rsPendapatanJenis['jenis_kendaraan']; ?></td>
        <td class="w3-border-right w3-center"><?php
		$year = substr($row_rsPendapatanJenis['masuk'],0,4);
		$mont = substr($row_rsPendapatanJenis['masuk'],5,2);
		$day = substr($row_rsPendapatanJenis['masuk'],8,2);
		$jam = substr($row_rsPendapatanJenis['masuk'],11,5);
		 ?> <?php echo $day."-".$mont."-".$year." ".$jam; ?>
		 </td>
        <td class="w3-border-right w3-center"><?php
		$year2 = substr($row_rsPendapatanJenis['keluar'],0,4);
		$mont2 = substr($row_rsPendapatanJenis['keluar'],5,2);
		$day2 = substr($row_rsPendapatanJenis['keluar'],8,2);
		$jam2 = substr($row_rsPendapatanJenis['keluar'],11,5);
		 ?> <?php echo $day2."-".$mont2."-".$year2." ".$jam2; ?></td>
        <td class="w3-border-right w3-center"><?php echo $row_rsPendapatanJenis['durasi']; ?></td>
        <td>Rp.<span class="w3-right"><?php echo number_format($row_rsPendapatanJenis['biaya'],0,",","."); ?></span></td>
      </tr>
      <?php } while ($row_rsPendapatanJenis = mysql_fetch_assoc($rsPendapatanJenis)); ?>
      <?php
	include("koneksi_sum.php");
      	$a1 = 0;
	  	$sql1 = mysqli_query($conn,"SELECT * FROM tb_parkir WHERE jenis_kendaraan = '$colname_rsPendapatanJenis' AND status='Keluar'");
...
<?php
mysql_free_result($rsPendapatanJenis);
?>
<?php if ($totalRows_rsPendapatanJenis == 0) { // Show if recordset empty ?>
  <div class="w3-row">
