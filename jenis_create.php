...
if (isset($_POST[$MM_flag])) {
  $MM_dupKeyRedirect="jenis_error.php";
  $loginUsername = $_POST['jenis_kendaraan'];
  $LoginRS__query = sprintf("SELECT jenis_kendaraan FROM tb_jenis_kendaraan WHERE jenis_kendaraan=%s", GetSQLValueString($loginUsername, "text"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO tb_jenis_kendaraan (id_jenis_kendaraan, jenis_kendaraan) VALUES (%s, %s)",
                       GetSQLValueString($_POST['id_jenis_kendaraan'], "text"),
...
if (isset($_GET['id_jenis_kendaraan'])) {
  $colname_rsJenisCreate = $_GET['id_jenis_kendaraan'];
}
mysql_select_db($database_koneksi, $koneksi);
$query_rsJenisCreate = sprintf("SELECT * FROM tb_jenis_kendaraan WHERE id_jenis_kendaraan = %s", GetSQLValueString($colname_rsJenisCreate, "text"));
...
$IDJenis = "123456789123456789123456789";
?>
...
<br><br><br><br>
  ...
  <a onClick="return confirm('Anda Yakin Ingin Keluar?')" href="<?php echo $logoutAction ?>" style="text-decoration:none"><li class="w3-hover-pale-blue"><i class="fa fa-sign-out fa-fw"></i> Keluar</li></a>
        ...
        <div class="w3-large w3-border-bottom" style="padding-bottom:8px;"><a style="cursor:pointer" onClick="window.history.back();document.getElementById('loading').style.display='block'"><i class="fa fa-arrow-left"></i></a> &nbsp;&nbsp;Tambah Jenis Kendaraan</div>
<form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
  <table width="100%" align="center" style="margin-top:8px;">
    <tr valign="baseline" style="display:none">
      <td align="left" valign="middle" nowrap="nowrap">Id_jenis_kendaraan:</td>
      <td><input type="text" name="id_jenis_kendaraan" value="<?php echo substr(str_shuffle($IDJenis),0,10) ?>" size="32" /></td>
    </tr>
    <tr valign="baseline">
      <td align="left" valign="middle" nowrap="nowrap">Jenis Kendaraan</td>
      <td><input type="text" class="w3-input w3-border w3-small" required name="jenis_kendaraan" value="" size="32" /></td>
    </tr>
